...
//hidenseek exposes this during One Versus All. Optional, so this plugin still loads without it.
...
public Plugin myinfo = 
{
  ...
  author = "Invex, Byte, cREANy0, Fredd, Kevin",
  description = "spawn protections",
...
public void OnPluginStart()
{
  ...
  //ConVar List
  CreateConVar("sm_easysp_version", PLUGIN_VERSION, "Version of 'Easy Spawn Protection' plugin", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_CHEAT|FCVAR_DONTRECORD);
...
public Action Event_OnPlayerSpawn(Handle event, const char[] name, bool dontBroadcast)
{
  ...
  //Wrong mode, but allow it if still inside the protection window.
...
{
  //OVA hands the T role over on every stab and the new T must be killable at once.
  //Checked here rather than at the two call sites: guarding only player_spawn left
  //the round-start loop still granting protection.
...
public Action RemoveProtection(Handle timer, any client)
{
  ...
  //One-shot timer: it must not be rescheduled.
  return Plugin_Stop;
}
